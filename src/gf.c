                              ...
                              size_t world, size_t *min_valid, size_t *max_valid)
{
    ...
    struct jl_typemap_assoc search = {(jl_value_t*)type, world, 1, jl_emptysvec, env.min_valid, env.max_valid};
    ...
    if (((jl_datatype_t*)unw)->isdispatchtuple) {
        ...
        if (ml) {
            ...
            ml_matches_visitor(ml, &env.match); // also matches all ambig and prior, as needed
        }
    }
    else {
